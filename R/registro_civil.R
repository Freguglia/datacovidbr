...
registro_civil <- function(state = "all", city_id = "all"){
  ...
  out <- data.frame()
  for(gender in c("M", "F")){
  api_url <- glue::glue("https://transparencia.registrocivil.org.br/api/covid-covid-registral?start_date=2020-03-16&end_date={lubridate::today()-1}&state={state}&city_id={city_id}&places[]=HOSPITAL&places[]=DOMICILIO&places[]=VIA_PUBLICA&places[]=OUTROS&chart=chart2&gender={gender}")
